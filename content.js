...
// OBSERVE STARTS
await waitForElement({ sel: chat_container(), callback: (el) => {
  dispatch("dredutils:chatContainer", { node: el });
} });
await waitForElement({ sel: motd_container, callback: (el) => {
  dispatch("dredutils:motdContainer", { node: el });
} });
await waitForElement({ sel: shipyard_ships(), callback: (el) => {
  dispatch("dredutils:shipyardShips", { node: el });
} });
await waitForElement({ sel: window_dark_container(), callback: (el) => {
...
await startMutationObserver(team_menu_container(), { from: "teamMenuContainer" });
// OBSERVE ENDS