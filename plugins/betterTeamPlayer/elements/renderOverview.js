...
const wireFilters = async (box, data) => {
  ...
  fOn.onchange = e => { state.online = e.target.checked; applyState(data); };
  fRank.onchange = e => { state.rank = e.target.value; applyState(data); };
  fHi.onchange = e => { state.highlight = e.target.checked; applyState(data); };
  fDim.onchange = e => { state.dimOffline = e.target.checked; applyState(data); };
